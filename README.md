...
- Локальный `onec-mcp-bridge 0.9.0` для подключения готового MCP-сервера `mcp-1c` к Cloudflare-приложению; development-профиль умеет безопасно читать полный BSL-модуль, отдельную процедуру/функцию и компактные XML-метаданные объекта, разрешать символы, находить ссылки, считать checksum и оценивать payload без изменения конфигурации 1С.
