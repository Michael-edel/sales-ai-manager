...
- Панель read-only проверок 1С в интерфейсе администратора и менеджера:
  ...
  - `Карточка партнера` — основная read-only кнопка по выбранной заявке; карточка партнера в 1С содержит адреса, договоры, контактные лица, банковские счета, взаимодействия и прочую информацию по компании;
...
- по выбранному CRM-клиенту основная read-only кнопка интерфейса открывает `Карточку партнера`; отдельные данные вроде договоров, адресов, контактных лиц и банковских счетов менеджер смотрит как часть этой карточки;
