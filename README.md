...
- Панель read-only проверок 1С в интерфейсе администратора и менеджера:
  ...
  - `Договоры клиента`, `Последние заказы`, `Счета клиента`, `Задолженность` — read-only проверки по CRM-клиенту выбранной заявки;
...
- по выбранному CRM-клиенту доступны read-only проверки договоров, последних заказов, счетов и задолженности; если в конфигурации отличаются имена документов/регистров, программа покажет понятную ошибку попыток;
