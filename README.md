...
- Локальный `onec-mcp-bridge` для подключения готового MCP-сервера `mcp-1c` к Cloudflare-приложению; development-профиль умеет безопасно читать полный BSL-модуль, отдельную процедуру/функцию и компактные XML-метаданные объекта без изменения конфигурации 1С.
