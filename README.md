...
4. Выберите папку: `Полученные`, `В работе`, `Закрытые` или `Удаленные`.
5. Нажмите «Открыть» — письмо раскроется сразу под выбранной строкой и будет помечено как просмотренное. Повторное нажатие «Свернуть» закрывает это письмо, а кнопка «Свернуть все» закрывает открытый просмотр.
...
8. Если отправитель точно не присылает заявки, отметьте галочку «Не показывать письма от этого отправителя». Адрес попадет в список скрытых отправителей, и письма от него больше не будут отображаться в рабочих папках. Восстановить адрес можно в блоке «Скрытые отправители» кнопкой «показывать».
...
```text
GET /api/email/messages?folder=inbox|in_work|done|trash
