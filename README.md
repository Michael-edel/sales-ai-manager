...
- Панель read-only проверок 1С в интерфейсе администратора и менеджера:
  - `Проверить 1C` — статус bridge и сведения о конфигурации 1С;
  - `Найти клиента в 1С` — поиск по названию клиента или БИН/ИИН в `Справочник.Партнеры` и `Справочник.Контрагенты`; формы `ТОО/ИП/АО/LLP` в начале или конце запроса дополнительно очищаются, поэтому `ТОО KBI Energy` ищется также как `KBI Energy`;
  - найденного клиента 1С можно привязать к CRM-клиенту выбранной заявки; если поиск вернул несколько KBI/ТОО, менеджер выбирает правильную карточку один раз, а программа сохраняет точную привязку партнера и/или контрагента;
  ...
  - `Открыть карточку клиента`, `Договоры клиента`, `Последние заказы`, `Заказы/счета`, `Взаимодействия`, `Задолженность`, `Условия оплаты`, `Адреса/доставка` — read-only проверки по CRM-клиенту выбранной заявки;
  - в текущей базе 1С счет на оплату считается печатной формой документа `Документ.ЗаказКлиента`; поэтому кнопка `Заказы/счета` читает именно `Заказ клиента`, а не отдельный документ счета;
  ...
  - `Команда 1С` — короткие команды обычным языком: `открой последний заказ KBI`, `счета KBI за июнь`, `долг KBI`, `условия оплаты KBI`, `адреса KBI`, `цены по привязанным товарам`;
...
- для KBI и других клиентов основной карточкой считается `Справочник.Партнеры`; в ней берутся договоры, документы, взаимодействия, условия оплаты, адреса и доставка, если база возвращает эти данные через MCP;
- в этой конфигурации 1С `Счет на оплату` — печатная форма `Документ.ЗаказКлиента`; поэтому read-only проверка `Заказы/счета` читает `Документ.ЗаказКлиента`;
- по выбранному CRM-клиенту доступны read-only проверки карточки партнера/контрагента, договоров, последних заказов, заказов/счетов, взаимодействий, задолженности, условий оплаты и адресов/доставки; если в конфигурации отличаются имена документов/регистров, программа покажет понятную ошибку попыток;
...
- frontend показывает найденных партнеров/контрагентов 1С как варианты и сохраняет выбранный вариант через `/api/crm/clients/:id/1c-counterparty`;
