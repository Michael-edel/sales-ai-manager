...
- Панель read-only проверок 1С в интерфейсе администратора и менеджера:
  ...
  - `Найти клиента в 1С` — поиск по названию клиента или БИН/ИИН в `Справочник.Контрагенты`; формы `ТОО/ИП/АО/LLP` в начале или конце запроса дополнительно очищаются, поэтому `ТОО KBI Energy` ищется также как `KBI Energy`;
  - найденного клиента 1С можно привязать к CRM-клиенту выбранной заявки; если поиск вернул несколько KBI/ТОО, менеджер выбирает правильную карточку один раз, а программа сохраняет точную привязку;
  - `Открыть карточку клиента`, `Договоры клиента`, `Последние заказы`, `Счета клиента`, `Задолженность`, `Условия оплаты`, `Адреса/доставка` — read-only проверки по CRM-клиенту выбранной заявки;
...
- Привязка точного контрагента 1С хранится в D1 у `crm_clients` и возвращается в карточке заявки.
...
- по выбранному CRM-клиенту доступны read-only проверки карточки контрагента, договоров, последних заказов, счетов, задолженности, условий оплаты и адресов/доставки; если в конфигурации отличаются имена документов/регистров, программа покажет понятную ошибку попыток;
...
- frontend показывает найденных контрагентов 1С как варианты и сохраняет выбранный вариант через `/api/crm/clients/:id/1c-counterparty`;
