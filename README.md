...
Файл формируется без изменения данных 1С: он берет номер/дату счета, извлеченные ИИ данные по товару и делает приложение по шаблону KBI:
...
- реквизиты покупателя `ТОО «KBI Energy Group»` и поставщика `ТОО «Michael»`.
