...
- Панель read-only проверок 1С в интерфейсе администратора:
...
- Серверные read-only бизнес-инструменты 1С, чтобы frontend не отправлял произвольные SQL-запросы напрямую:
  - `GET /api/1c/status`;
...
- рабочие кнопки интерфейса используют серверные read-only endpoints `/api/1c/...`, а не произвольный `execute_query` из браузера;
