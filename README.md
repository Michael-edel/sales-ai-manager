...
- Панель read-only проверок 1С в интерфейсе администратора и менеджера:
  ...
  - для `Справочник.Контрагенты` в УТ Казахстан используется поле `БИН_ИИН`, а не российское поле `ИНН`;
  ...
  - `Полная карточка` — основная read-only кнопка по выбранной заявке; программа собирает в одном экране срез карточки партнера из 1С: основную информацию, договоры, банковские счета, адреса/контакты, заказы-счета, взаимодействия, взаиморасчеты и условия оплаты;
