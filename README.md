...
- Локальный `onec-mcp-bridge 0.9.1` для подключения готового MCP-сервера `mcp-1c` к Cloudflare-приложению; development-профиль умеет безопасно читать BSL/метаданные и получать ограниченную выборку live-записей регистра через `read_register_records`, не открывая Inspector произвольный `execute_query`.
